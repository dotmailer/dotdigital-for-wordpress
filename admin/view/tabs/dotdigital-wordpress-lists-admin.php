		...
		<form method="<?php echo esc_attr( $form->get_method() ); ?>" action="<?php echo esc_attr( $form->get_action() ); ?>">
			...
			<table class="wp-list-table widefat fixed form-table">
				...
				<?php foreach ( $form->get_grouped_inputs() as $group_id => $inputs ) : ?>
					<tr id="<?php echo esc_attr( $group_id ); ?>" class="dragger toggle-inputs" >
						<?php list($id_input, $label_input, $visible_input) = $inputs; ?>
						<th scope="row">
							<span class="handle ui-sortable-handle"><img src="<?php echo esc_url( plugins_url( '../../../assets/large.png', __FILE__ ) ); ?>" class="drag_image" /></span>
							<?php $id_input->render(); ?>
						</th>
						<td class="list-column"><?php echo esc_html( $label_input->get_label() ); ?></td>
						<td><?php $label_input->render(); ?></td>
						<td><?php $visible_input->render(); ?></td>
					</tr>
				<?php endforeach; ?>
				</tbody>
