...
class Dotdigital_Wordpress_Integration_Insights {
	...
	private function get_dotdigital_widgets() {
		...
		foreach ( $sidebars_widgets as $sidebar_id => $widget_ids ) {
			...
			foreach ( $widget_ids as $widget_id ) {
				if ( isset( $widget_instances[ $widget_id ] ) ) {// Add to array.
					...
					if ( isset( $sidebars_widget_instances[ $sidebar_id ][ $widget_id ]['content'] ) && strpos( $sidebars_widget_instances[ $sidebar_id ][ $widget_id ]['content'], 'dotdigital' ) ) {
						...
						$widget_options[ 'widget-' . count( $widget_options ) ] = array(
							'widget_name' => 'Dotdigital Signup Form [Legacy]',
							'widget_area' => $sidebar_id,
						);
					}
				}
			}
	...
	private function get_configuration() {
		...
		$configuration['lists'] = false !== $lists ? array_combine(
			array_column( $lists, 'id' ),
			array_reverse( array_keys( $lists ) )
		) : array();
