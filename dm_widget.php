...
class DM_Widget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array(
			'classname' => 'dm_widget',
			'description' => __( 'Deprecated - please use the new Dotdigital Signup Form block.', 'dm_widget' ),
		);
		$control_ops = array( 'id_base' => 'dm_widget' );
		parent::__construct( 'dm_widget', __( 'Dotdigital Signup Form [Legacy]', 'dm_widget' ), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$redirection_arg = ( isset( $args['redirection'] ) ) ? $args['redirection'] : null;
		...
		if ( empty( $creds ) ) {
			?>
			<div>
				<?php echo "<p class='error_message'>Dotdigital Signup Form: invalid API credentials. Please check your API credentials in the plugin settings.</p>"; ?>
			</div>
			<?php
			return;
		}
		if ( empty( $msgs ) ) {
			?>
			<div>
				<?php echo "<p class='error_message'>Dotdigital Signup Form: no messages configured. Please set form messages via the 'Messages' tab in the plugin settings.</p>"; ?>
			</div>
			<?php
			return;
		}
		...
		if ( isset( $_POST['dm_submit_btn'] ) ) {
			...
			if ( empty( $formErrors ) ) {
				...
				$api = new DotdigitalConnect(
					array(
						'username' => $dm_api_credentials['dm_API_username'],
						'password' => $dm_api_credentials['dm_API_password'],
					)
				);
				...
				if ( $email && $redirection && empty( $formErrors ) ) {
					$string = '<script type="text/javascript">';
					$string .= 'window.location = "' . $redirection . '"';
					$string .= '</script>';
					echo $string;
				}
			}
		...
		<form class="dotMailer_news_letter" style="margin:5px 0 10px 0;" method="post" action ="<?php get_bloginfo( 'url' ); ?>" >
			<?php
			if ( $showdesc ) {
				echo '<p>Please complete the fields below:</p>';}
			?>
			 ...
			 * Redirection
			 *
			 * If widget instance is defined via a shortcode, use the redirection arg.
			 * If not, look up the stored option.
			 */
			$redirect_target = false;
			if ( $redirection_arg ) {
				$redirect_target = $redirection_arg;
			} else {
				$option = get_option( 'dm_redirections', array() );
				if ( array_key_exists( 'page', $option ) ) {
					$redirect_target = get_permalink( $option['page'] );
			...
			<?php if ( $redirect_target ) { ?>
				<input type="hidden" name="dotMailer_redirection" value="<?php echo $redirect_target; ?>" />
			<?php } ?>
